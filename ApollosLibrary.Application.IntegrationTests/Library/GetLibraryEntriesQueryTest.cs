...
namespace ApollosLibrary.Application.IntegrationTests.Library
{
    ...
    public class GetLibraryEntriesQueryTest : TestBase
    {
        ...
        public GetLibraryEntriesQueryTest(TestFixture fixture) : base(fixture)
        {
        ...
        public async Task GetLibraryEntries()
        {
            ...
            var command = new GetLibraryEntriesQuery();

            var result = await _mediatr.Send(command);

            result.Should().NotBeNull();
            result.LibraryEntries.Should().HaveCount(2);
            result.LibraryEntries.Should().BeEquivalentTo(new List<LibraryEntryListItemDTO>()
            {
                new LibraryEntryListItemDTO()
                {
                    Author = "Author",
                ...
                {
                    Author = "Author et al.",
                    EntryId = entity2.EntryId,
                    Quantity = entity2.Quantity,
                    Title = entity2.Book.Title,
                }
            });            
        }
    }
